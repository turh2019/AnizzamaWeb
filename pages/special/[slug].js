...
const SpecialPage = ({page, ep,slugs}) => {
 ...
 var title_ = page.title;
  ...
  return (
    <>
      ...
      <Head>
      <title>{title_}</title>
          <link rel="canonical" href={"https://www.anizzama.com/special/"+page.title}/>
          <meta property="og:title" content={title_}/>
          <meta property="og:description" content={ogDescription}/>
          <meta property="description" content={description}/>
          <meta property="og:url" content= {"https://www.anizzama.com/special/"+slugs.slug}/>
...
export async function getStaticPaths() {
  const posts = await getPageFormat('special');

  const paths = posts.filter((p,index)=>{
    if(p.node.format == "special"){
      return p;
    }
