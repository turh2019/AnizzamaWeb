...
import { LinksTo, PostWidget, ShowProfile,EditProfile} from '../../components/getComponents';
...
export async function getStaticProps({ params }) {
  const data = await GetmyProfileSlug(params.name);
...
export async function getStaticPaths() {
  ...
  return {
    paths: profiles.map((  { name } ) => ({ params: { name } })),
    fallback: true,
  };
}
