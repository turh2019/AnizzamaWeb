...
const CategoriesSearch = ({posts}) => {
    ...
    var CatgoryottHandel = (e) =>{
        ...
        getvalue(Array.isArray(e)?e.map(x =>  x):[]);
      
    }

    var CatgoryottHandel_ = (e,y) =>{
        
        getvalue(Array.isArray(e)?e.filter(x => x.label!= y? x:null):[]);
      
    }

    var FansubHandel = (e) =>{
        
        GetFansub(Array.isArray(e)?e.map(x => x):[]);
      
    }
  
    var FansubHandel_ = (e, y) =>{
        
        GetFansub(Array.isArray(e)?e.filter(x => x.label!= y? x:null):[]);
    }
    ...
    return (
       <div className=' p-4 lg:p-0 text-white   ' >
            <div className=' lg:grid lg:grid-cols-4 pl-5  gap-4' dir="rtl">
                ...
                <div className='  pb-7  text-withe-4'>
                    ...
                    <div className='cursor-pointer'>
                        <Select hideSelectedOptions ={true}  value={ Fansub?.filter((p) => {return p})}    styles=" lg:p-0 text-white"  placeholder="בחר/י..."  autoFocus={true}   tabSelectsValue={true} isRtl={true}  isMulti options={fansub} onChange={FansubHandel} className ="px-8 h-30 w-60 text-sky-600  font-bold  shadow-2xl   ease-in duration-300 accent-pink-500  scroll-ml-6" closeMenuOnSelect={false}></Select>
                    </div>
                ...
                <div className='  pb-7  text-withe-4'>
                    ...
                    <div className='cursor-pointer'>
                    <Select hideSelectedOptions value={projectStatus != ""? [{value:projectStatus,label:projectStatus}]:""}  placeholder="בחר/י..." isRtl={true} components={animatedComponents}  isClearable={true} options={ProjectStatus} onChange={(e)=>(SetProjectStatus(e?e.label:""))}  className ="px-8 h-30 w-60 text-sky-600  font-bold  shadow-2xl   ease-in duration-300 accent-pink-500  scroll-ml-6" closeMenuOnSelect={true}></Select>
                    </div>
                ...
                <div className='  pb-7  text-withe px-4'>
                    ...
                    <div className='cursor-pointer'>
                        <Select hideSelectedOptions value={formt != "" ?[{value:formt,label:formt}]:""} placeholder="בחר/י..."isRtl={true} components={animatedComponents}  isClearable={true} options={formts} onChange={(e)=>(setformt(e?e.label:""))} className =" px-8 h-30 w-60 text-sky-600  font-bold  shadow-2xl    accent-pink-500  scroll-ml-6" closeMenuOnSelect={true}></Select>
                    </div>
                           ...
                           {displayvalue ? 
                            displayvalue.map((G)=>(
                                <span key={G.label} className='break-ward text-center   shadow-size-[15px] rounded-lg   font-bold  text-[12px] p-1 px-2 out hadow-lg shadow-[#3E2E88] cursor-pointer transition duration-500 ease transform hover:-translate-y-1 px-1 rounded-lg  mb-4 ml-4  bg-[#3E2E88]' >
                                    {G.label}
                                    <button className='text-center float-right transition duration-500 ease transform hover:bg-[#FF0000] px-1 text-[12px]  rounded-full  ' onClick={(e)=>(CatgoryottHandel_(displayvalue,G.label))}>x</button>
                                </span>
                            ...
                            {Fansub ? 
                            Fansub.map((G)=>(
                                <span key={G.value} className='break-ward text-center   shadow-size-[15px] rounded-lg   font-bold  text-[12px] p-1 px-2 out hadow-lg shadow-[#3E2E88] cursor-pointer transition duration-500 ease transform hover:-translate-y-1  rounded-lg  mb-4 ml-4  bg-[#3E2E88]' >
                                    {G.value}
                                   
                                    <button className='text-center float-right transition duration-500 ease transform hover:bg-[#FF0000] px-1 text-[12px]  rounded-full  ' onClick={(e)=>(FansubHandel_(Fansub,G.value))}>x</button>
                                </span>
                            ))
                            :""}
