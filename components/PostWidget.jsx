      ...
      {relatedPosts.map((post, index) => (
        <div key={index} className="flex items-right w-full mb-6 text-white ">
          <div className="flex-grow ml-10">
            ...
            <div className='flex justify-center text-center transition duration-700 text-center color-white  mb-8  hover:text-[#4864F6]  font-semibold  mx-3'>
               <Link target="_blank" href={`/post/${post.slug}`} className="text-md flex justify-center " key={index}>
                 {post.title}
                 </Link>
            </div>
          </div>
