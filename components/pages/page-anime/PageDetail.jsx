...
const PageDetail = ({ post , ep ,page ,slugs}) => {
  ...
  const getContentFragment = (index, text, obj, type) => {
    ...
    switch (obj.type) {
      ...
      case 'paragraph'://" "
        return <p key={index} className="mb-5 mx-2 text-lg text-white ">{modifiedText.map((item, i) => <React.Fragment key={i}>{item}</React.Fragment>)}</p>;
      ...
      case 'heading-four'://h4
        ...
        case 'link'://h4
        return  <a target="_blank" key={index} href={obj.href}><Link target="_blank" key={index} href={obj.href} ><span className ="transition duration-700   text-blue-300  text-center   mb-8 cursor-pointer hover:text-cover_color   ">{obj.title}</span></Link></a>;
