                ...
                <div className=' shadow-lg rounded-lg p-8 pb-12 mb-8'>
                     <h3 className=' text-xl mb-8 font-semibold border-b pb-4 text-white  '>
                      ...
                      {comments.map((cooment,index1) =>(
                          !cooment.isBelongs  &&
                            <div className='relative p-2 'key={index1 + cooment.id}>                         
                                <div className="border-gray-400 border-[2px] rounded-full shadow-lg absolute inline-block  bottom-10 top-10 -right-7 h-20 w-20 ">
                                    ...
                                    <div div key={cooment.id} className='text-gray-400  relative border-gray-400 mb-4 pb-4  px-3  rounded-lg relative  right-8'>
                                        <div className='bg-cover_color border-2 text-gray-400 border-gray-400    px-3  rounded-lg'>
                                            <p className='mt-2 mb-4  md:grid-cols-4 pb-2'>
                                                ...
                                                {console.log(Author?.name +" " + cooment?.author?.name)}
                                            ...
                                            {clickEdit == index1 && cooment.author.name === profile.name && <button className='hover:text-white my-4'   onClick={ (e)=>( CliCkEditCommenteHandel(false,false,true,cooment.id,false,false,editComments) ,SetClickEdit(-1),setEditComments(""))} >פרסם תגובה</button>}
                                            {clickEdit == index1 && cooment.author.name === profile.name &&  <button className='hover:text-white my-4 m-2'   onClick={ (e)=>( SetClickEdit(-1),setEditComments(""))} >בטל</button>}                                                                                                 
                                            {isLogin && clickEdit != index1&& cooment.author?.name === profile.name&&<button className={' pt-4  mr-4 text-white hover:scale-125 select-none '}  onClick = {(e) =>(SetClickEdit(index1),setEditComments(parse(cooment.comment)))}><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="w-5 h-5"><path d="M5.433 13.917l1.262-3.155A4 4 0 017.58 9.42l6.92-6.918a2.121 2.121 0 013 3l-6.92 6.918c-.383.383-.84.685-1.343.886l-3.154 1.262a.5.5 0 01-.65-.65z" /><path d="M3.5 5.75c0-.69.56-1.25 1.25-1.25H10A.75.75 0 0010 3H4.75A2.75 2.75 0 002 5.75v9.5A2.75 2.75 0 004.75 18h9.5A2.75 2.75 0 0017 15.25V10a.75.75 0 00-1.5 0v5.25c0 .69-.56 1.25-1.25 1.25h-9.5c-.69 0-1.25-.56-1.25-1.25v-9.5z" /></svg></button>}
                                            {clickEdit != index1 && cooment.author?.name === profile?.name && <button onClick={(e) => (setIsOpen(true), setcommentId(cooment.id))} className='text-white m-2 mr-4 hover:scale-125  hover:text-rose-600 ' ><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="w-5 h-5"><path fill-rule="evenodd" d="M8.75 1A2.75 2.75 0 006 3.75v.443c-.795.077-1.584.176-2.365.298a.75.75 0 10.23 1.482l.149-.022.841 10.518A2.75 2.75 0 007.596 19h4.807a2.75 2.75 0 002.742-2.53l.841-10.52.149.023a.75.75 0 00.23-1.482A41.03 41.03 0 0014 4.193V3.75A2.75 2.75 0 0011.25 1h-2.5zM10 4c.84 0 1.673.025 2.5.075V3.75c0-.69-.56-1.25-1.25-1.25h-2.5c-.69 0-1.25.56-1.25 1.25v.325C8.327 4.025 9.16 4 10 4zM8.58 7.72a.75.75 0 00-1.5.06l.3 7.5a.75.75 0 101.5-.06l-.3-7.5zm4.34.06a.75.75 0 10-1.5-.06l-.3 7.5a.75.75 0 101.5.06l.3-7.5z" clip-rule="evenodd" /></svg></button>      }                                                                                                             
                                           
                                            <button className={isLogin && cooment.plus.filter((e) => e.name == profile.name).length > 0 ? 'md:scale-125 scale-75 my-4 float-left md:mr-2 sd:scale-50 text-emerald-400 hover:scale-150 hover:text-white ':'md:scale-125 scale-75 my-4 float-left md:mr-2 text-white hover:scale-150 hover:text-emerald-400 select-none'}  onClick={ (e)=>(isLogin ? CliCkEditCommenteHandel(true,false,false,cooment.id,cooment.plus.filter((e) => e.name == profile.name).length > 0 || cooment.minus.filter((e) => e.name == profile.name).length > 0, cooment.plus.filter((e) => e.name == profile.name).length > 0):CliCkEditCommenteHandel())} ><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="w-6 h-6"><path d="M7.493 18.75c-.425 0-.82-.236-.975-.632A7.48 7.48 0 016 15.375c0-1.75.599-3.358 1.602-4.634.151-.192.373-.309.6-.397.473-.183.89-.514 1.212-.924a9.042 9.042 0 012.861-2.4c.723-.384 1.35-.956 1.653-1.715a4.498 4.498 0 00.322-1.672V3a.75.75 0 01.75-.75 2.25 2.25 0 012.25 2.25c0 1.152-.26 2.243-.723 3.218-.266.558.107 1.282.725 1.282h3.126c1.026 0 1.945.694 2.054 1.715.045.422.068.85.068 1.285a11.95 11.95 0 01-2.649 7.521c-.388.482-.987.729-1.605.729H14.23c-.483 0-.964-.078-1.423-.23l-3.114-1.04a4.501 4.501 0 00-1.423-.23h-.777zM2.331 10.977a11.969 11.969 0 00-.831 4.398 12 12 0 00.52 3.507c.26.85 1.084 1.368 1.973 1.368H4.9c.445 0 .72-.498.523-.898a8.963 8.963 0 01-.924-3.977c0-1.708.476-3.305 1.302-4.666.245-.403-.028-.959-.5-.959H4.25c-.832 0-1.612.453-1.918 1.227z" /></svg></button>
                                            <span className=' my-4 float-left text-white font-bold md:m-4 md:scale-125 scale-75' dir="ltr">{cooment.plus.length - cooment.minus.length}</span>
                                            <button className={isLogin && cooment.minus.filter((e) => e.name == profile.name).length > 0 ? 'md:scale-125 scale-75  my-4 float-left md:ml-2 scale-125 text-rose-600 md:hover:scale-150   hover:scale-120  hover:text-white':'md:scale-125 scale-75 md:hover:scale-150 my-4 float-left ml-2 text-white hover:scale-120 hover:text-rose-600 select-none'}  onClick={ (e)=>(isLogin ? CliCkEditCommenteHandel(false,true,false,cooment.id ,cooment.plus.filter((e) => e.name == profile.name).length > 0 || cooment.minus.filter((e) => e.name == profile.name).length > 0, cooment.minus.filter((e) => e.name == profile.name).length > 0 ):CliCkEditCommenteHandel())} ><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="w-6 h-6"><path d="M15.73 5.25h1.035A7.465 7.465 0 0118 9.375a7.465 7.465 0 01-1.235 4.125h-.148c-.806 0-1.534.446-2.031 1.08a9.04 9.04 0 01-2.861 2.4c-.723.384-1.35.956-1.653 1.715a4.498 4.498 0 00-.322 1.672V21a.75.75 0 01-.75.75 2.25 2.25 0 01-2.25-2.25c0-1.152.26-2.243.723-3.218C7.74 15.724 7.366 15 6.748 15H3.622c-1.026 0-1.945-.694-2.054-1.715A12.134 12.134 0 011.5 12c0-2.848.992-5.464 2.649-7.521.388-.482.987-.729 1.605-.729H9.77a4.5 4.5 0 011.423.23l3.114 1.04a4.5 4.5 0 001.423.23zM21.669 13.773c.536-1.362.831-2.845.831-4.398 0-1.22-.182-2.398-.52-3.507-.26-.85-1.084-1.368-1.973-1.368H19.1c-.445 0-.72.498-.523.898.591 1.2.924 2.55.924 3.977a8.959 8.959 0 01-1.302 4.666c-.245.403.028.959.5.959h1.053c.832 0 1.612-.453 1.918-1.227z" /></svg></button>
