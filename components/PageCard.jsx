...
const PageCard = ({ post }) => (
  <div className="bg-[#261D78] text-white shadow-lg rounded-lg lg:p-8 border-2  border-[#18154E] scale-[1]  lg:scale-[0.90]" >
      ...
      <div className="font-medium text-white-700">
        ...
        <span className="align-middle">{moment(post.createdAt).format('MMM DD, YYYY')}</span>
          ...
          <h1 className="transition duration-700 text-center color-white  mb-8 cursor-pointer hover:text-[#4864F6] text-3xl font-semibold  ">
            <Link href={`/page_/${post.slug}`} key ={post.slug}>{post.title}</Link>
          </h1>
